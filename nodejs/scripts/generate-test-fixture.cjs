...
const buildCandidateEntries = (count, docxAvailable) => {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    let extension = '.pdf';
    const includeSensitiveToken = index === 0;

    if (!includeSensitiveToken && docxBudget > 0 && index % 5 === 0) {
      extension = '.docx';
      docxBudget -= 1;
    } else if (!includeSensitiveToken && rtfBudget > 0 && index % 5 === 1) {
      extension = '.rtf';
      rtfBudget -= 1;
    }
